...
const RAW_ALGEBRA_CASES: RawAlgebraCase[][] = [
  ...
  [
    ["Write rule plus three", "Section 1 - Write the Rule: Input 1, 2, 3, 4, 5 gives output 4, 5, 6, 7, 8. Complete Output = n + __.", "Compare each output with its input.", ["3", "4", "1"], "3", numbers("Rule from table.", ["1->4", "2->5", "3->6", "4->7", "5->8"]), "write-the-rule", "Write the rule", ["uses-first-output-as-rule"]],
    ["Write rule double", "Section 1 - Write the Rule: Input 1, 2, 3, 4 gives output 2, 4, 6, 8. Which rule fits?", "Each output is two times the input.", ["Output = 2n", "Output = n + 2", "Output = n + 4"], "Output = 2n", numbers("Rule from table.", ["1->2", "2->4", "3->6", "4->8"]), "write-the-rule", "Write the rule", ["additive-for-multiplicative"]],
    ["Use rule n plus three", "Section 2 - Use the Rule: Output = n + 3. What outputs match n = 1, 2, 3, 4?", "Add 3 to each n value.", ["4, 5, 6, 7", "3, 6, 9, 12", "1, 2, 3, 4"], "4, 5, 6, 7", numbers("Use rule n + 3.", ["n=1", "n=2", "n=3", "n=4"]), "use-the-rule", "Use the rule", ["uses-input-as-output"]],
    ["Use rule two n", "Section 2 - Use the Rule: Output = 2n. What outputs match n = 1, 2, 3, 4?", "Double each n value.", ["2, 4, 6, 8", "3, 4, 5, 6", "1, 4, 9, 16"], "2, 4, 6, 8", numbers("Use rule 2n.", ["n=1", "n=2", "n=3", "n=4"]), "use-the-rule", "Use the rule", ["adds-two-instead"]],
    ["Taxi model match", "Section 3 - Match the Situation: A taxi costs $5 plus $2 per kilometre. Which rule matches?", "The 5 is fixed and the 2 changes with each kilometre.", ["Cost = 5 + 2n", "Cost = 12n", "Height = 3n"], "Cost = 5 + 2n", numbers("Taxi model.", ["$5 start", "$2 per km"]), "match-the-situation", "Match the situation", ["swaps-fixed-and-changing"]],
    ["Movie ticket match", "Section 3 - Match the Situation: Movie tickets cost $12 each. Which rule matches n tickets?", "There is no extra starting cost; multiply tickets by 12.", ["Cost = 12n", "Cost = 5 + 2n", "Savings = 10 + 5n"], "Cost = 12n", numbers("Movie ticket model.", ["$12 each", "n tickets"]), "match-the-situation", "Match the situation", ["adds-fixed-cost"]],
    ["Plant growth match", "Section 3 - Match the Situation: A plant grows 3 cm per week. Which rule matches n weeks?", "Each week adds another 3 cm.", ["Height = 3n", "Height = n + 3", "Savings = 10 + 5n"], "Height = 3n", numbers("Plant growth model.", ["3 cm per week", "n weeks"]), "match-the-situation", "Match the situation", ["additive-for-rate"]],
    ["Savings model match", "Section 3 - Match the Situation: Savings start at $10 and grow by $5 per week. Which rule matches?", "Keep the starting 10 and add 5 dollars for each week.", ["Savings = 10 + 5n", "Savings = 5 + 10n", "Savings = 15n"], "Savings = 10 + 5n", numbers("Savings model.", ["$10 start", "$5 per week"]), "match-the-situation", "Match the situation", ["swaps-rate-and-start"]],
    ["Sticker packet model", "Section 4 - Build the Model: Each packet contains 4 stickers. Which rule gives total stickers for n packets?", "Use 4 stickers for every packet.", ["Stickers = 4n", "Stickers = n + 4", "Stickers = 4 - n"], "Stickers = 4n", numbers("Sticker packets.", ["4 per packet", "n packets"]), "build-the-model", "Build the model", ["uses-addition-for-groups"]],
    ["Game cost model", "Section 4 - Build the Model: A game has an $8 entry fee plus $2 per ride. Which rule gives cost for n rides?", "Start with 8, then add 2 for each ride.", ["Cost = 8 + 2n", "Cost = 2 + 8n", "Cost = 10n"], "Cost = 8 + 2n", numbers("Game cost model.", ["$8 entry", "$2 per ride"]), "build-the-model", "Build the model", ["swaps-fixed-and-changing"]],
    ["Substitute n plus four", "Section 5 - Find the Missing Value: y = n + 4. If n = 7, what is y?", "Substitute 7 for n, then add 4.", ["11", "7", "3"], "11", numbers("Substitution card.", ["y = n + 4", "n = 7"]), "find-the-missing-value", "Find the missing value", ["does-not-substitute"]],
    ["Substitute three n", "Section 5 - Find the Missing Value: y = 3n. If n = 5, what is y?", "Multiply 3 by 5.", ["15", "8", "35"], "15", numbers("Substitution card.", ["y = 3n", "n = 5"]), "find-the-missing-value", "Find the missing value", ["adds-instead-of-multiplies"]],
    ["Substitute two n plus one", "Section 5 - Find the Missing Value: y = 2n + 1. If n = 4, what is y?", "Double 4, then add 1.", ["9", "8", "6"], "9", numbers("Substitution card.", ["y = 2n + 1", "n = 4"]), "find-the-missing-value", "Find the missing value", ["misses-final-addition"]],
    ["Graph rule n plus two", "Section 6 - Graph the Relationship: Rule y = n + 2. Which point should be plotted for n = 3?", "Find y first, then plot n as the input and y as the output.", ["(3, 5)", "(5, 3)", "(3, 2)"], "(3, 5)", numbers("Graph from rule y = n + 2.", ["n=3", "y=?"]), "graph-the-relationship", "Graph the relationship", ["reverses-coordinates"]],
    ["Cupcake fundraiser", "Section 7 - Real-Life Algebra: Money = 3n for cupcakes sold. How much money is raised for 20 cupcakes?", "Each cupcake raises 3 dollars.", ["$60", "$23", "$40"], "$60", numbers("Cupcake fundraiser.", ["Money = 3n", "n = 20"]), "real-life-algebra", "Real-life algebra", ["adds-rate-instead"]],
    ["Explain algebra usefulness", "Section 8 - Think and Explain: Why is algebra useful for modelling relationships?", "A rule can predict new values without rebuilding every table.", ["It helps predict values efficiently", "It only works for one example", "It avoids checking the context"], "It helps predict values efficiently", numbers("Relationship prediction.", ["context", "rule", "prediction"]), "think-and-explain", "Think and explain", ["rule-purpose-gap"]],
  ],
  [
    ["Explain rule", "Section 1 - Explain the Rule: Input 1, 2, 3, 4, 5 gives output 4, 6, 8, 10, 12. What rule fits?", "Compare how the output changes as the input increases.", ["Output = 2n + 2", "Output = n + 3", "Output = 4n"], "Output = 2n + 2", numbers("Rule explanation table.", ["1->4", "2->6", "3->8", "4->10", "5->12"]), "explain-the-rule", "Explain the rule", ["uses-first-row-only"]],
    ["How know rule", "Section 1 - Explain the Rule: How do you know Output = 2n + 2 fits the table?", "Check the rule on more than one row.", ["It works for each input-output pair", "It only matches the first output", "It makes every output equal 2"], "It works for each input-output pair", numbers("Check rule across rows.", ["2n+2", "1->4", "5->12"]), "explain-the-rule", "Explain the rule", ["weak-justification"]],
    ["Check x plus five", "Section 2 - Check the Solution: x + 5 = 12. Student answer: x = 7. Is it correct?", "Substitute 7 for x and check the equation.", ["Correct, because 7 + 5 = 12", "Incorrect, because 7 - 5 = 12", "Incorrect, because x should be 17"], "Correct, because 7 + 5 = 12", numbers("Check solution.", ["x + 5 = 12", "x = 7"]), "check-the-solution", "Check the solution", ["does-not-substitute"]],
    ["Check n minus four", "Section 2 - Check the Solution: n - 4 = 9. Student answer: n = 13. Is it correct?", "Substitute 13 for n.", ["Correct, because 13 - 4 = 9", "Incorrect, because 13 + 4 = 9", "Incorrect, because n should be 5"], "Correct, because 13 - 4 = 9", numbers("Check solution.", ["n - 4 = 9", "n = 13"]), "check-the-solution", "Check the solution", ["wrong-inverse-check"]],
    ["Explain relationship", "Section 3 - Explain the Relationship: Input 1, 2, 3, 4 gives output 3, 6, 9, 12. What is always true?", "Compare each output with its input.", ["The output is always 3 times the input", "The output is always 3 more than the input", "The input is always 3 times the output"], "The output is always 3 times the input", numbers("Relationship table.", ["1->3", "2->6", "3->9", "4->12"]), "explain-relationship", "Explain the relationship", ["additive-for-multiplicative"]],
    ["Relationship rule", "Section 3 - Explain the Relationship: Which rule describes input n and output 3, 6, 9, 12?", "Use multiplication by 3.", ["Output = 3n", "Output = n + 3", "Output = n - 3"], "Output = 3n", numbers("Rule for relationship.", ["n", "3n"]), "explain-relationship", "Explain the relationship", ["operation-translation-error"]],
    ["Generalise stars", "Section 4 - Generalise the Pattern: A growing star pattern has Group 1 = 2, Group 2 = 4, Group 3 = 6, Group 4 = 8. How many stars are in Group 10?", "Find the group rule, then use Group 10.", ["20", "12", "10"], "20", groups("Growing star groups.", [2, 4, 6, 8, 10], ["G1", "G2", "G3", "G4", "G5"]), "generalise-pattern", "Generalise the pattern", ["uses-next-term-only"]],
    ["Explain group ten", "Section 4 - Generalise the Pattern: How can you explain Group 10 for the 2, 4, 6, 8 pattern?", "Use the relationship between group number and stars.", ["Each group has 2 times the group number", "Each group always has 2 stars", "Add the group number to 10"], "Each group has 2 times the group number", groups("Generalising star pattern.", [2, 4, 6, 8], ["G1", "G2", "G3", "G4"]), "generalise-pattern", "Generalise the pattern", ["no-general-rule"]],
    ["Explain graph increase", "Section 5 - Explain the Graph: A graph has points (1,3), (2,6), (3,9), (4,12). What happens when x increases by 1?", "Read the change in y between neighbouring points.", ["y increases by 3", "y increases by 1", "y decreases by 3"], "y increases by 3", numbers("Graph interpretation.", ["(1,3)", "(2,6)", "(3,9)", "(4,12)"]), "explain-graph", "Explain the graph", ["graph-trend-confusion"]],
    ["Graph rule", "Section 5 - Explain the Graph: Which rule matches points (1,3), (2,6), (3,9), (4,12)?", "Each y-value is 3 times x.", ["y = 3x", "y = x + 3", "y = x - 3"], "y = 3x", numbers("Graph rule.", ["x", "y", "3x"]), "explain-graph", "Explain the graph", ["uses-y-intercept-only"]],
    ["Spot table error", "Section 6 - Spot the Error: Input 1, 2, 3, 4 gives output 2, 5, 8, 12. Student rule: Add 3. Is the rule correct?", "Check every jump in the output row.", ["No, because 8 to 12 adds 4", "Yes, every output adds 3", "No, because 2 to 5 adds 2"], "No, because 8 to 12 adds 4", numbers("Error table.", ["1->2", "2->5", "3->8", "4->12"]), "spot-the-error", "Spot the error", ["checks-first-differences-only"]],
    ["Correct table error", "Section 6 - Spot the Error: If the rule is add 3 and outputs start 2, 5, 8, what should the fourth output be?", "Continue adding 3.", ["11", "12", "10"], "11", numbers("Correct the output.", [2, 5, 8, "?"]), "spot-the-error", "Spot the error", ["does-not-correct-error"]],
    ["Create general rule", "Section 7 - Create and Generalise: A pattern has Group 1 = 3, Group 2 = 6, Group 3 = 9, Group 4 = 12. What rule describes Group n?", "Connect the group number to the number of objects.", ["3n", "n + 3", "n - 3"], "3n", groups("Create and generalise.", [3, 6, 9, 12], ["G1", "G2", "G3", "G4"]), "create-and-generalise", "Create and generalise", ["additive-for-multiplicative"]],
    ["Predict group ten", "Section 7 - Create and Generalise: Using the rule 3n, how many objects are in Group 10?", "Substitute 10 into 3n.", ["30", "13", "10"], "30", groups("Predict Group 10.", [3, 6, 9, 12], ["G1", "G2", "G3", "G4"]), "create-and-generalise", "Create and generalise", ["uses-visible-groups-only"]],
    ["Checking importance", "Section 8 - Reflect and Explain: Why is checking your answer important?", "Choose the explanation that matches the worksheet reflection.", ["It helps find mistakes and confirm reasoning", "It makes the answer longer", "It replaces explaining the rule"], "It helps find mistakes and confirm reasoning", numbers("Reflection prompt.", ["answer", "check", "reason"]), "reflect-and-explain", "Reflect and explain", ["checking-purpose-gap"]],
    ["Rules predict", "Section 8 - Reflect and Explain: How can a rule help predict future values?", "A rule lets you calculate values beyond the table.", ["Use the rule with a new input", "Only copy the last value", "Ignore the relationship"], "Use the rule with a new input", numbers("Future value prediction.", ["rule", "new input", "prediction"]), "reflect-and-explain", "Reflect and explain", ["prediction-gap"]],
  ],
];
