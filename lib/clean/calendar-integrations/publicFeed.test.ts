...
function buildStore(options: {
  ...
  const store: CalendarFeedReadStore = {
    async findActiveByTokenHash(tokenHash) {
      ...
      return tokenHash === activeHash ? { id: "subscription-1", familyId } : null;
    },
...
describe("private Apple Calendar feed lookup", () => {
  it("returns the exact active subscription family", async () => {
    ...
    const result = await loadAppleCalendarFeed("active-token", fake.store);
  ...
  it("returns null for an invalid or revoked token without loading events", async () => {
    ...
    expect(await loadAppleCalendarFeed("invalid-token", invalid.store)).toBeNull();
    ...
    expect(await loadAppleCalendarFeed("revoked-token", revoked.store)).toBeNull();
  ...
  it("invalidates the old token immediately after rotation", async () => {
    ...
    expect(await loadAppleCalendarFeed("old-token", fake.store)).toBeNull();
    expect(await loadAppleCalendarFeed("new-token", fake.store)).not.toBeNull();
    fake.revoke();
    expect(await loadAppleCalendarFeed("new-token", fake.store)).toBeNull();
  });
});
