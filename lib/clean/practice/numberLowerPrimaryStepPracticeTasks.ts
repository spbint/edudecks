import { NUMBER_LOWER_PRIMARY_STEP_ASSESSMENTS } from "@/lib/clean/assessments/numberLowerPrimaryStepAssessments";
import type { NumberPracticeTask } from "@/lib/clean/practice/numberPowersRootsPracticeModules";

export type LowerPrimaryStepPracticeDefinition = {
  key: string;
  stepNumber: number;
  stepKey: string;
...
function makeTask(
  assessment: (typeof NUMBER_LOWER_PRIMARY_STEP_ASSESSMENTS)[number],
  index: number,
): NumberPracticeTask {
  ...
  return {
    ...
    supportPrompt:
      "Use the picture or number cards first, then choose the matching answer.",
    ...
    visualSupport: visual(
      assessmentItem.visualSupport?.description ||
        "early-number|caption=Use the visual support.|groups=10|labels=counters",
    ),
  };
}

export const NUMBER_LOWER_PRIMARY_STEP_PRACTICES =
  NUMBER_LOWER_PRIMARY_STEP_ASSESSMENTS.map((assessment) => {
  ...
  }) satisfies LowerPrimaryStepPracticeDefinition[];
