...
const STORAGE_KEY = "edudecks.forum.v1";
...
export function readLocalForum(): LocalForumStore {
  if (typeof window === "undefined") {
    return { categories: DEFAULT_FORUM_CATEGORIES, threads: [], posts: [] };
  }
...
export async function loadCommunityHomeData(viewerId: string | null) {
  ...
  try {
    ...
    if (categoriesResp.error) {
      ...
      return {
        categories: buildCategorySummaries(local.categories, local.threads, local.posts, viewerId),
    ...
    const threadsResp = await supabase
      .from("forum_threads")
      .select("id,category_id,user_id,title,body,is_pinned,created_at,updated_at")
...
export async function loadCategoryPageData(slug: string, viewerId: string | null) {
  ...
  try {
    ...
    return {
      category,
      threads: buildThreadSummaries(threads, posts, viewerId),
      source: "database" as const,
    };
  } catch {
    return localCategory
