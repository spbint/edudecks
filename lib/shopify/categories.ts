...
const MARKETPLACE_CATEGORY_HANDLES = new Set<string>(MARKETPLACE_CATEGORIES.map((category) => category.handle));

export function isApprovedMarketplaceCollectionHandle(handle: string) {
  return MARKETPLACE_CATEGORY_HANDLES.has(handle);
}

export function isMarketplaceProductEligible(product: Pick<ShopifyProductSummary, "collections">) {
  return product.collections.some((collection) => isApprovedMarketplaceCollectionHandle(collection.handle));
}
