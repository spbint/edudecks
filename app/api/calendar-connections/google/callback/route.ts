...
import { completeGoogleCalendarConnection } from "@/lib/clean/calendar-integrations/googleConnectionService";
import { googleCalendarSettingsReturnUrl } from "@/lib/clean/calendar-integrations/googleOAuth";
...
function redirect(result: "connected" | "error") {
  try {
    const response = NextResponse.redirect(googleCalendarSettingsReturnUrl(result), 303);
    response.headers.set("cache-control", "private, no-store");
    response.headers.set("referrer-policy", "no-referrer");
    return response;
  } catch {
    return new NextResponse("Google Calendar is not configured.", {
      status: 503,
      headers: {
