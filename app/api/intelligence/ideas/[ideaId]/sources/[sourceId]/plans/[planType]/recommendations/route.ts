...
const RECOMMENDATION_ROUTE = "/api/intelligence/ideas/[ideaId]/sources/[sourceId]/plans/[planType]/recommendations";
...
async function contextFor(planTypeValue: string, diagnostics: IntelligenceRouteDiagnostics) {
  ...
  if (!isLearningPlanType(planTypeValue)) {
    diagnostics.stageFailure("authenticated_context", { name: "InvalidInput", code: "invalid_input", message: "Invalid plan type.", status: 400 });
    return { response: NextResponse.json({ code: "invalid_input", error: "Choose either a lesson or unit plan." }, { status: 400 }) };
  }
  try {
...
function createService(client: Parameters<typeof createSupabaseApprovedPlanRevisionRepository>[0], diagnostics?: IntelligenceRouteDiagnostics) {
  return createRecommendationService({
    approvedPlanRepository: createSupabaseApprovedPlanRevisionRepository(client, diagnostics),
    ownedResourceRepository: createSupabaseFamilyOwnedResourceRepository(client, diagnostics),
    interactionRepository: createSupabaseRecommendationInteractionRepository(client, diagnostics),
    diagnostics,
  });
}
...
export async function GET(request: Request, context: RouteContext) {
  const diagnostics = createIntelligenceRouteDiagnostics(RECOMMENDATION_ROUTE);
  const { ideaId, sourceId, planType: planTypeValue } = await context.params;
  let scoped;
  try {
    scoped = await contextFor(planTypeValue, diagnostics);
  } catch (error) {
  ...
  if (!exactPlan) { diagnostics.responseReady(); return NextResponse.json({ code: "invalid_input", error: "An exact approved plan revision is required." }, { status: 400 }); }
  try {
    const repository = createSupabaseApprovedPlanRevisionRepository(scoped.auth.client, diagnostics);
    ...
    if (!snapshot) { diagnostics.responseReady(); return NextResponse.json({ code: "not_approved", error: "That exact approved revision is not available." }, { status: 404 }); }
    const service = createService(scoped.auth.client, diagnostics);
...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (!exactPlan) { diagnostics.responseReady(); return NextResponse.json({ code: "invalid_input", error: "An exact approved plan revision is required." }, { status: 400 }); }
  let body: unknown;
  try { body = await request.json(); } catch { diagnostics.responseReady(); return NextResponse.json({ code: "invalid_input", error: "An interaction event is required." }, { status: 400 }); }
  if (!body || typeof body !== "object" || Array.isArray(body)) { diagnostics.responseReady(); return NextResponse.json({ code: "invalid_input", error: "An interaction event is required." }, { status: 400 }); }
  const event = body as Record<string, unknown>;
  const eventType = event.eventType;
  const recommendationId = typeof event.recommendationId === "string" ? event.recommendationId : "";
  if (!recommendationId || typeof eventType !== "string" || !EVENT_TYPES.has(eventType as RecommendationInteractionEventType)) { diagnostics.responseReady(); return NextResponse.json({ code: "invalid_input", error: "A valid recommendation event is required." }, { status: 400 }); }
  try {
    const repository = createSupabaseApprovedPlanRevisionRepository(scoped.auth.client, diagnostics);
    const snapshot = await repository.getApprovedRevisionForUser(scoped.auth.user.id, ideaId, sourceId, scoped.planType, exactPlan.planId, exactPlan.revisionNumber);
    if (!snapshot) { diagnostics.responseReady(); return NextResponse.json({ code: "not_approved", error: "That exact approved revision is not available." }, { status: 404 }); }
