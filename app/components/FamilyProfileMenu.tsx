...
function routeTitle(pathname: string) {
  if (pathname === "/family") return "EduDecks Family";
  if (pathname === "/calendar") return "EduDecks Family";
  if (pathname === "/capture") return "EduDecks Family";
  if (pathname === "/planner") return "EduDecks Family";
  if (pathname === "/portfolio") return "EduDecks Family";
  if (pathname === "/reports") return "EduDecks Family";
  if (pathname === "/profile") return "EduDecks Family";
  if (pathname === "/settings") return "EduDecks Family";
  if (pathname === "/community") return "EduDecks Family";
  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        ...
        className={cx(
          "inline-flex items-center gap-3 rounded-full border border-slate-200 bg-white px-3 py-2 shadow-[0_8px_20px_rgba(15,23,42,0.05)] transition",
          "hover:bg-slate-50",
        )}
        aria-haspopup="menu"
        aria-expanded={open}
          ...
          <div className="px-3 py-3">
            <MenuSection
              ...
              items={[
                ...
                { label: "Default learner", href: "/profile#family-identity" },
              ]}
            ...
            <div className="mt-2 border-t border-slate-200 px-2 pt-3">
              <Link
                ...
                className="flex w-full items-center justify-between rounded-2xl px-3 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50"
              >
                <span>Sign out</span>
      ...
      <div className="grid gap-1">
        {items.map((item) => (
          <Link
            ...
            href={item.href}
            onClick={onNavigate}
            className="flex items-center justify-between rounded-2xl px-3 py-3 text-sm font-semibold text-slate-800 transition hover:bg-slate-50"
          >
