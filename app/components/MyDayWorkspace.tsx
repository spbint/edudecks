...
=======
  if (!isActiveLearner) return "Ready to review";
  if (blocksCount > 0 && evidenceCount > 0) return `${blocksCount} planned · ${evidenceCount} captured`;
  if (blocksCount > 0) return `${blocksCount} planned today`;
  if (evidenceCount > 0) return `${evidenceCount} captured today`;
>>>>>>> d620063e4a741c4550b39bfcb0badf02f4d6f58e
...
=======
        ...
        <div className="text-[22px] font-bold tracking-[-0.03em] text-slate-950">Nothing is planned for today yet</div>
        <p className="max-w-[64ch] text-[14px] leading-6 text-slate-600">
          Nothing is broken. My Day will fill in once you add a learning block, pull in your calendar rhythm, or capture something that already happened today for {learnerName}.
>>>>>>> d620063e4a741c4550b39bfcb0badf02f4d6f58e
        </p>
      </div>
...
=======
          <div><span className="font-semibold text-slate-900">1.</span> Add one clear block in My Plan.</div>
          <div><span className="font-semibold text-slate-900">2.</span> Use My Calendar when you want your weekly rhythm to guide the day.</div>
          <div><span className="font-semibold text-slate-900">3.</span> Capture a real learning moment if today has already started.</div>
>>>>>>> d620063e4a741c4550b39bfcb0badf02f4d6f58e
        </div>
      </div>
...
=======
        <Link href={`/my-plan?date=${encodeURIComponent(todayYmd)}`} className="inline-flex items-center justify-center rounded-full bg-slate-950 px-5 py-3 text-[13px] font-semibold text-white transition hover:bg-slate-800">
          Shape today in My Plan
        </Link>
        <Link href="/calendar" className="inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-5 py-3 text-[13px] font-semibold text-slate-700 transition hover:bg-slate-50">
          Review My Calendar rhythm
        </Link>
>>>>>>> d620063e4a741c4550b39bfcb0badf02f4d6f58e
        <Link
...
=======
            canCapture ? "border-slate-200 bg-white text-slate-700 hover:bg-slate-50" : "pointer-events-none border-slate-100 bg-slate-50 text-slate-400",
>>>>>>> d620063e4a741c4550b39bfcb0badf02f4d6f58e
          ].join(" ")}
        >
          Capture from today
...
=======
        You do not need a perfect plan to begin. One planned block or one captured learning moment is enough to make today visible.
>>>>>>> d620063e4a741c4550b39bfcb0badf02f4d6f58e
      </p>
    </section>
  );
...
=======
      <div className="mb-3 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Today at a glance</div>
      <div className="grid gap-3 text-[13px]">
        <div className="flex items-center justify-between gap-3"><span className="text-slate-500">Selected learner</span><span className="font-bold text-slate-950">{learnerName}</span></div>
        <div className="flex items-center justify-between gap-3"><span className="text-slate-500">Planned blocks</span><span className="font-bold text-slate-950">{loading ? "…" : blocksCount}</span></div>
        <div className="flex items-center justify-between gap-3"><span className="text-slate-500">Captured today</span><span className="font-bold text-slate-950">{loading ? "…" : capturedCount}</span></div>
        <div className="rounded-[16px] border border-slate-100 bg-slate-50 p-3">
          <div className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-400">Next best action</div>
          <div className="mt-1 font-bold text-slate-950">{nextAction}</div>
        </div>
        <div className="flex flex-wrap gap-2 pt-1">
          <Link href={`/my-plan?date=${encodeURIComponent(todayYmd)}`} className="inline-flex items-center justify-center rounded-full bg-slate-950 px-4 py-2 text-[12px] font-semibold text-white transition hover:bg-slate-800">
            Open My Plan
          </Link>
>>>>>>> d620063e4a741c4550b39bfcb0badf02f4d6f58e
          <Link
...
=======
  const canonicalReady = Boolean(workspace.userId) && workspace.storageMode === "database" && Boolean(workspace.profile?.id) && workspace.profile.id !== "local" && Boolean(activeLearner?.id);
>>>>>>> d620063e4a741c4550b39bfcb0badf02f4d6f58e
...
<<<<<<< HEAD
  ...
  const preset: FrameworkPreset | null = effectiveConfig
    ...
    : null;
=======
  const learnerSelectorState: HomeSurfaceState = workspaceLoading ? "loading" : hasLearners ? workspace.storageMode === "database" ? "derived" : "placeholder" : "empty";
  const dayState: HomeSurfaceState = workspaceLoading || loadingDay ? "loading" : !hasLearners || !hasActiveLearner ? "empty" : canonicalReady ? blocksToday.length || evidenceRows.length ? "live" : "empty" : "placeholder";
>>>>>>> d620063e4a741c4550b39bfcb0badf02f4d6f58e

  const effectiveConfig = hasActiveLearner ? resolveEffectiveLearnerLearningConfig(workspace.profile, activeLearner) : null;
  const preset: FrameworkPreset | null = effectiveConfig ? presetFromFrameworkSelection({ country: effectiveConfig.country, frameworkId: effectiveConfig.frameworkId, jurisdictionId: effectiveConfig.jurisdictionId }) : null;
...
>>>>>>> d620063e4a741c4550b39bfcb0badf02f4d6f58e
  ...
  const dayView = activeLearnerId ? buildMyDayView({ date: todayYmd, learnerId: activeLearnerId, blocks: blocksToday, programs, evidenceRows, now: today }) : null;
...
=======
  const quickCaptureHref = activeLearnerId ? `/capture?learner=${encodeURIComponent(activeLearnerId)}&date=${encodeURIComponent(todayYmd)}` : "/capture";
  const portfolioHref = activeLearnerId ? `/my-portfolio?learner=${encodeURIComponent(activeLearnerId)}` : "/my-portfolio";
  const headerState: HomeSurfaceState = !hasActiveLearner ? "empty" : canonicalReady ? "live" : "placeholder";
>>>>>>> d620063e4a741c4550b39bfcb0badf02f4d6f58e
...
=======
    return {
      id: learner.id,
      label: learner.label,
      note: `${learner.yearLabel || learner.year_band || "Learner"} · ${learnerStatusText({ active: Boolean(activeLearner), blocksCount: isActiveLearner ? blocksToday.length : 0, evidenceCount: isActiveLearner ? evidenceToday.length : 0, isActiveLearner })}`,
    };
  });

  const heroAsideText = hasActiveLearner && !workspaceLoading
    ? `${activeLearnerName}: ${blocksToday.length} planned block${blocksToday.length === 1 ? "" : "s"}, ${evidenceToday.length} captured learning moment${evidenceToday.length === 1 ? "" : "s"} today.`
    : "Choose a learner to see today's blocks, capture entry points, and the next useful step.";
>>>>>>> d620063e4a741c4550b39bfcb0badf02f4d6f58e
...
=======
        <LearnerSelector familyName={workspace.profile.family_display_name || "Your family"} learners={learnerOptions} activeLearnerId={activeLearner?.id} onSelectLearner={setActiveLearner} state={learnerSelectorState} />
>>>>>>> d620063e4a741c4550b39bfcb0badf02f4d6f58e
...
=======
            blocks={<>{dayView?.blocks.map((block) => (
              <TodayLearningBlockCard
                key={block.id}
                block={block}
                planHref={`/my-plan?date=${encodeURIComponent(todayYmd)}`}
...
<<<<<<< HEAD
                ...
                />
=======
                <div className="grid gap-4">{[0, 1].map((item) => <div key={item} className="h-44 animate-pulse rounded-[22px] border border-slate-200 bg-slate-100" />)}</div>
              ) : blocksToday.length === 0 ? (
                <TodayEmptyGuidance learnerName={activeLearnerName} todayYmd={todayYmd} quickCaptureHref={quickCaptureHref} canCapture={canCapture} />
>>>>>>> d620063e4a741c4550b39bfcb0badf02f4d6f58e
              ) : (
                <MyDayEmptyState />
              )
...
=======
            <TodayAtAGlancePanel learnerName={activeLearnerName} blocksCount={blocksToday.length} capturedCount={evidenceToday.length} dayState={dayState} canCapture={canCapture} todayYmd={todayYmd} quickCaptureHref={quickCaptureHref} />
            <MyDayNextUpCard
              block={dayView?.nextUp ?? null}
              learnerName={activeLearnerName}
              planHref={dayView?.nextUp ? `/my-plan?date=${encodeURIComponent(todayYmd)}&block=${encodeURIComponent(dayView.nextUp.id)}` : `/my-plan?date=${encodeURIComponent(todayYmd)}`}
              captureHref={dayView?.nextUp ? `/capture?learner=${encodeURIComponent(activeLearnerId)}&date=${encodeURIComponent(todayYmd)}&block=${encodeURIComponent(dayView.nextUp.id)}` : quickCaptureHref}
>>>>>>> d620063e4a741c4550b39bfcb0badf02f4d6f58e
              canCapture={canCapture}
            />
