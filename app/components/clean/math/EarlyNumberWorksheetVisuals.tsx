...
export function isOperationsStep2ShareCompareActivity(id: string, stepKey?: string | null) {
  return (
    safe(stepKey) === "share-compare-and-notice-simple-differences" ||
    safe(id).startsWith("operations-step-2-assess-") ||
    safe(id).startsWith("operations-step-2-practice-")
  );
}
...
export function isStep48EstimationBoundsActivity(id: string, stepKey?: string | null) {
  return (
    safe(stepKey) === "apply-estimation-rounding-and-bounds" ||
    safe(id).startsWith("number-step-48-assess-") ||
    safe(id).startsWith("number-step-48-practice-")
  );
}
...
export function isStep53ExactFractionsPiActivity(id: string, stepKey?: string | null) {
  return (
    safe(stepKey) ===
      "calculate-exactly-with-fractions-and-multiples-of-pi-where-appropriate" ||
    safe(id).startsWith("number-step-53-assess-") ||
    safe(id).startsWith("number-step-53-practice-")
  );
}

export function isStep54PercentageChangeActivity(id: string, stepKey?: string | null) {
  return (
    safe(stepKey) === "work-with-percentage-change-growth-and-decay" ||
    safe(id).startsWith("number-step-54-assess-") ||
    safe(id).startsWith("number-step-54-practice-")
  );
}
...
export function isStep56AlgebraGraphActivity(id: string, stepKey?: string | null) {
  return (
    safe(stepKey) === "use-number-skills-in-algebraic-and-graphical-contexts" ||
    safe(id).startsWith("number-step-56-assess-") ||
    safe(id).startsWith("number-step-56-practice-")
  );
}

export function isStep57FinancialModellingActivity(id: string, stepKey?: string | null) {
  return (
    safe(stepKey) === "solve-financial-and-real-world-modelling-problems" ||
    safe(id).startsWith("number-step-57-assess-") ||
    safe(id).startsWith("number-step-57-practice-")
  );
}

export function isStep58AccuracyRoundingActivity(id: string, stepKey?: string | null) {
  return (
    safe(stepKey) === "interpret-limits-of-accuracy-and-rounding" ||
    safe(id).startsWith("number-step-58-assess-") ||
    safe(id).startsWith("number-step-58-practice-")
  );
}
    ...
    return (
      <span
        ...
        style={{
          ...
          boxShadow: size <= 10 ? "none" : "inset 0 0 0 3px rgba(255,255,255,0.22)",
        }}
      >
        {[24, 52, 72].map((left, dotIndex) => (
          <span
            key={`chip-${dotIndex}`}
            style={{
              position: "absolute",
              left: `${left}%`,
              top: `${dotIndex === 1 ? 62 : 30}%`,
              width: "14%",
              height: "14%",
              borderRadius: "50%",
              background: "#451a03",
    ...
    return (
      <span
        ...
        style={{
          ...
          borderBottom: `${Math.max(5, Math.round(size * 0.28))}px solid #facc15`,
...
function isOperationsStep2SharingPrompt(prompt: string, visual: EarlyNumberVisualModel) {
  const source = `${prompt} ${visual.caption}`.toLowerCase();
  return source.includes("share") || source.includes("equally");
}

function isOperationsStep2StoryPrompt(prompt: string, visual: EarlyNumberVisualModel) {
  const source = `${prompt} ${visual.caption}`.toLowerCase();
  return (
    source.includes("gives") ||
    source.includes("left") ||
    source.includes("take") ||
...
export function renderOperationsStep2WorksheetPromptVisual({
  prompt,
  visual,
}: {
  prompt: string;
  ...
  const first = visual.groupCounts[0];
  const second = visual.groupCounts[1];

  if (
    isOperationsStep2StoryPrompt(prompt, visual) &&
    first !== undefined &&
    second !== undefined &&
    !isOperationsStep2SharingPrompt(prompt, visual)
  ) {
    return renderStep9WorksheetPromptVisual({ prompt, visual });
  }
  ...
  if (
    ...
    const total = first;
    const groupCount = Math.max(1, second);
    const eachGroupCount = total % groupCount === 0 ? total / groupCount : null;
    const totalLabel = visual.labels[0] || `${total} objects`;
    ...
    return (
      ...
      >
        <div style={{ color: "#1e3a8a", fontSize: 14, fontWeight: 850, lineHeight: 1.45 }}>
          Share the objects equally so each group gets the same amount.
        </div>
        <div
          style={{
            border: "1px solid #dbeafe",
        ...
        >
          <div style={{ display: "grid", gap: 6 }}>
            ...
            <EarlyNumberWorksheetObjectGroupCard count={total} label={totalLabel} />
          </div>
          <div aria-hidden="true" style={{ color: "#15803d", fontSize: 26, fontWeight: 950, textAlign: "center" }}>
            -&gt;
          </div>
          <div style={{ display: "grid", gap: 6 }}>
            <div style={{ color: "#15803d", fontSize: 12, fontWeight: 900, textTransform: "uppercase", textAlign: "center" }}>
              {groupCount} equal groups
            </div>
            <div
              style={{
                display: "grid",
                gridTemplateColumns: `repeat(${Math.min(groupCount, 4)}, minmax(82px, 1fr))`,
                gap: 8,
              }}
            >
              {Array.from({ length: groupCount }, (_, index) => (
                <div
                  key={`operations-share-${index}`}
                  style={{
                ...
                >
                  <div style={{ color: "#166534", fontSize: 12, fontWeight: 900, textAlign: "center" }}>
                    Group {index + 1}
                  </div>
                  {eachGroupCount !== null ? (
                    <EarlyNumberWorksheetObjectGroupCard count={eachGroupCount} label={`${eachGroupCount} each`} />
                  ) : (
                    <div style={{ color: "#166534", fontSize: 20, fontWeight: 950, textAlign: "center" }}>?</div>
                  )}
                </div>
              ))}
            </div>
  ...
  if (
    ...
    const firstLabel = visual.labels[0] || `Group A`;
    const secondLabel = visual.labels[1] || `Group B`;
    ...
    return (
      ...
      >
        <div style={{ color: "#1e3a8a", fontSize: 14, fontWeight: 850, lineHeight: 1.45 }}>
          Compare the two groups. Look for more, less, same, or how many more.
        </div>
        <div
          style={{
            border: "1px solid #dbeafe",
        ...
        >
          <EarlyNumberWorksheetObjectGroupCard count={first} label={firstLabel} />
          <EarlyNumberWorksheetObjectGroupCard count={second} label={secondLabel} />
        </div>
        <div
          style={{
            border: `1px solid ${same ? "#bbf7d0" : "#fed7aa"}`,
            borderRadius: 16,
            background: same ? "#f0fdf4" : "#fff7ed",
            color: same ? "#166534" : "#c2410c",
            padding: "10px 12px",
            fontSize: 13,
            fontWeight: 850,
            lineHeight: 1.45,
        ...
        >
          {same ? "The groups match: they are the same." : `Difference: ${difference} more in the larger group.`}
        </div>
      </div>
    );
  }
...
export function renderOperationsStep2WorksheetOptionCard({
  option,
  selected = false,
}: {
  option: string;
  ...
  const pictureCount = lower.match(/\btwo groups of\s+(\d+)/)?.[1];
  if (pictureCount) {
    return (
      <div
      ...
      >
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 6 }}>
          <EarlyNumberWorksheetObjectGroupCard count={Number(pictureCount)} label="Group 1" variant="compact" />
          <EarlyNumberWorksheetObjectGroupCard count={Number(pictureCount)} label="Group 2" variant="compact" />
        </div>
  ...
  const label = lower.includes("same")
    ? "Same"
    : lower.includes("group a") || lower.includes("group b")
      ? "Compare"
      : lower.includes("joining")
        ? "Joining"
        : lower.includes("taking")
          ? "Taking away"
          : lower.includes("more") || lower.includes("less")
            ? "Compare"
            : "Choice";
...
function getStep48VisualMode(prompt: string) {
  const lower = safe(prompt).toLowerCase();
  if (lower.includes("1 significant figure")) return "1 significant figure";
  if (lower.includes("nearest 1,000") || lower.includes("nearest 1000")) return "Nearest 1,000";
  if (lower.includes("decimal place")) return "Decimal rounding";
  if (lower.includes("lower") || lower.includes("upper") || lower.includes("bounds")) {
    return "Lower and upper bounds";
  }
  if (lower.includes("about") || lower.includes("estimate")) return "Estimate first";
  if (lower.includes("sensible") || lower.includes("reasonable")) return "Reasonableness check";
  return "Estimation and bounds";
}

export function renderStep48WorksheetPromptVisual({
  prompt,
  visual,
}: {
  prompt: string;
  ...
  const values = visual.numberCards.length
    ...
    : ["298 + 412", "300 + 400", "700"];
  const labels = values.map((_, index) => safe(visual.labels[index]) || `Part ${index + 1}`);
  const mode = getStep48VisualMode(prompt);
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ color: "#1e3a8a", fontSize: 14, fontWeight: 850, lineHeight: 1.45 }}>
          Estimate first, then use rounding and bounds to judge whether an
          answer is sensible within the limits of accuracy.
        </div>
        <span
          style={{
            border: "1px solid #bfdbfe",
      ...
      >
        ...
        >
          <span>Estimate: round to friendly numbers to check the size of an answer.</span>
          <span>Bounds: lower and upper limits show the possible range before rounding.</span>
          <span>Limits of accuracy: rounded values are useful, but not exact.</span>
        </div>
      </div>
    </div>
  );
...
export function renderStep48WorksheetOptionCard({
  option,
  selected = false,
}: {
  option: string;
  ...
  const lower = normalized.toLowerCase();
  const label = lower.includes("about")
    ? "Estimate"
    : lower.includes("and")
      ? "Bounds"
...
function getStep53Section(prompt: string, visual: EarlyNumberVisualModel) {
  const text = `${prompt} ${visual.caption} ${visual.numberCards.join(" ")}`.toLowerCase();
  if (text.includes("sector") || text.includes("wheel") || text.includes("pizza")) return "Real-world exact answer";
  if (text.includes("circle") || text.includes("radius") || text.includes("diameter") || text.includes("area")) {
    return "Circle formula";
  }
  if (text.includes("pi")) return "Exact pi expression";
  if (text.includes("recipe") || text.includes("tank") || text.includes("rice")) return "Fraction context";
  if (text.includes("expression")) return "Choose the expression";
  return "Exact fraction";
}

export function renderStep53WorksheetPromptVisual({
  prompt,
  visual,
}: {
  prompt: string;
  ...
  const cards = visual.numberCards.length ? visual.numberCards : visual.labels;
  const section = getStep53Section(prompt, visual);
  const primary = cards[0] ?? "exact calculation";
  const secondary = cards[1] ?? "simplest exact form";
  const tertiary = cards[2] ?? "";
    ...
    >
      ...
      >
        ...
        >
          ...
          <div style={{ marginTop: 5, fontSize: 14, fontWeight: 800 }}>
            Simplify fractions and combine like pi terms. Do not round pi unless asked.
          </div>
        </div>
        <div
          style={{
        ...
        >
          <div style={{ fontSize: 12, fontWeight: 950 }}>Formula cue</div>
          <div style={{ marginTop: 5, fontSize: 14, fontWeight: 800 }}>
...
export function renderStep53WorksheetOptionCard({
  option,
  selected = false,
}: {
  option: string;
  ...
  const lower = normalized.toLowerCase();
  const label = lower.includes("pi")
    ? "Exact pi"
    : normalized.includes("/")
      ? "Fraction"
      : lower.includes("cm") || lower.includes("m")
        ? "Context"
        : lower.includes("x") || lower.includes("+") || lower.includes("-")
          ? "Expression"
          : "Exact answer";
...
function getStep54Section(prompt: string, visual: EarlyNumberVisualModel) {
  const text = `${prompt} ${visual.caption} ${visual.numberCards.join(" ")}`.toLowerCase();
  if (text.includes("compound") || text.includes("^") || text.includes("each year")) {
    return text.includes("decay") || text.includes("decrease") || text.includes("loses")
      ? "Compound decay"
      : "Compound growth";
  }
  if (text.includes("successive") || text.includes("then") || text.includes("later")) {
    return "Successive changes";
  }
  if (text.includes("multiplier")) return "Choose a multiplier";
  if (text.includes("decrease") || text.includes("loses")) return "Percentage decrease";
  if (text.includes("increase") || text.includes("grows")) return "Percentage increase";
  return "Percentage change";
}

export function renderStep54WorksheetPromptVisual({
  prompt,
  visual,
}: {
  prompt: string;
  ...
  const cards = visual.numberCards.length ? visual.numberCards : visual.labels;
  const section = getStep54Section(prompt, visual);
  const primary = cards[0] ?? "original x multiplier";
  const secondary = cards[1] ?? "new value";
  const tertiary = cards[2] ?? "";
    ...
    >
        ...
        >
          <div
            style={{
              ...
              padding: "14px 10px",
              textAlign: "center",
              fontSize: primary.length > 28 ? 17 : 26,
              fontWeight: 950,
              lineHeight: 1.15,
            }}
          >
          ...
          <div
            style={{
              ...
              padding: "14px 10px",
              textAlign: "center",
              fontSize: secondary.length > 28 ? 17 : 26,
              fontWeight: 950,
              lineHeight: 1.15,
            }}
          >
      ...
      >
        ...
        >
          <div style={{ fontSize: 12, fontWeight: 950 }}>Remember</div>
          <div style={{ marginTop: 5, fontSize: 14, fontWeight: 800 }}>
            Increase: multiply by 1 + p/100. Decrease: multiply by 1 - p/100.
          </div>
        </div>
        <div
          style={{
        ...
        >
          <div style={{ fontSize: 12, fontWeight: 950 }}>Think about it</div>
          <div style={{ marginTop: 5, fontSize: 14, fontWeight: 800 }}>
            Repeated changes multiply. They are not added or subtracted from the original.
          </div>
        </div>
      </div>
...
export function renderStep54WorksheetOptionCard({
  option,
  selected = false,
}: {
  option: string;
  ...
  const lower = normalized.toLowerCase();
  const label = /^-?\d?\.?\d+$/.test(normalized)
    ? "Value"
    : lower.includes("£") || lower.includes("$")
      ? "Money"
      : lower.includes("less") || lower.includes("more") || lower.includes("original")
        ? "Reasoning"
        : lower.includes("g") || lower.includes("subscribers")
          ? "Context"
          : "Choice";
...
function getStep56Section(prompt: string, visual: EarlyNumberVisualModel) {
  const text = `${prompt} ${visual.caption} ${visual.numberCards.join(" ")}`.toLowerCase();
  if (text.includes("gradient") || text.includes("y-intercept") || text.includes("line")) return "Coordinate graph";
  if (text.includes("graph") || text.includes("distance") || text.includes("savings")) return "Graph interpretation";
  if (text.includes("rate") || text.includes("per hour") || text.includes("per month")) return "Rate of change";
  if (text.includes("equation") || text.includes("solve")) return "Solve equation";
  if (text.includes("when x") || text.includes("when a") || text.includes("when b")) return "Substitution";
  if (text.includes("word") || text.includes("number")) return "Algebraic context";
  return "Simplify expression";
}

export function renderStep56WorksheetPromptVisual({
  prompt,
  visual,
}: {
  prompt: string;
  ...
  const cards = visual.numberCards.length ? visual.numberCards : visual.labels;
  const section = getStep56Section(prompt, visual);
  const primary = cards[0] ?? "algebraic model";
  const secondary = cards[1] ?? "result";
  const tertiary = cards[2] ?? "";
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        ...
        <div style={{ border: "1px solid #bbf7d0", borderRadius: 999, background: "#f0fdf4", color: "#166534", padding: "7px 11px", fontSize: 13, fontWeight: 900 }}>
          Number skills in algebra
        </div>
      </div>

      <div style={{ border: "1px solid #dbeafe", borderRadius: 18, background: "#f8fbff", padding: 14, display: "grid", gap: 10 }}>
        <div style={{ color: "#1e3a8a", fontSize: 13, fontWeight: 900 }}>
          Algebra/graph working card
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) auto minmax(0, 1fr)", alignItems: "center", gap: 10 }}>
          <div style={{ border: "2px solid #bfdbfe", borderRadius: 16, background: "#ffffff", color: "#1e3a8a", padding: "14px 10px", textAlign: "center", fontSize: primary.length > 32 ? 16 : 26, fontWeight: 950, lineHeight: 1.15 }}>
            {primary}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 10 }}>
        <div style={{ border: "1px solid #bae6fd", borderRadius: 16, background: "#f0f9ff", padding: 12, color: "#075985" }}>
          <div style={{ fontSize: 12, fontWeight: 950 }}>Remember</div>
          <div style={{ marginTop: 5, fontSize: 14, fontWeight: 800 }}>
            Collect like terms, expand brackets, and keep equations balanced.
          </div>
        </div>
        <div style={{ border: "1px solid #e0e7ff", borderRadius: 16, background: "#eef2ff", padding: 12, color: "#3730a3" }}>
          <div style={{ fontSize: 12, fontWeight: 950 }}>Graph cue</div>
          <div style={{ marginTop: 5, fontSize: 14, fontWeight: 800 }}>
            Gradient is rise/run. In y = mx + c, m is gradient and c is y-intercept.
          </div>
        </div>
      </div>
...
export function renderStep56WorksheetOptionCard({
  option,
  selected = false,
}: {
  option: string;
  ...
  const lower = normalized.toLowerCase();
  const label = lower.includes("x =") || lower.includes("y =")
    ? "Equation"
    : lower.includes("km") || lower.includes("per")
      ? "Rate"
      : lower.includes("£")
        ? "Context"
        : lower.includes("x") || lower.includes("a") || lower.includes("y")
          ? "Algebra"
          : "Value";
...
function getStep57Section(prompt: string, visual: EarlyNumberVisualModel) {
  const text = `${prompt} ${visual.caption} ${visual.numberCards.join(" ")}`.toLowerCase();
  if (text.includes("best value") || text.includes("better value") || text.includes("pack") || text.includes("store") || text.includes("bag")) {
    return "Best buy";
  }
  if (text.includes("budget") || text.includes("spends") || text.includes("change") || text.includes("profit")) {
    return "Budgeting";
  }
  if (text.includes("worker") || text.includes("machine") || text.includes("journey") || text.includes("hours")) {
    return "Time and work";
  }
  if (text.includes("litre") || text.includes("millilitre") || text.includes("mass") || text.includes("area") || text.includes("capacity")) {
    return "Measurement model";
  }
  if (text.includes("discount") || text.includes("percentage") || text.includes("interest") || text.includes("reduced")) {
    return "Percent and money";
  }
  if (text.includes("first step") || text.includes("modelling") || text.includes("decision")) {
    return "Modelling decision";
  }
  return "Real-world model";
}

export function renderStep57WorksheetPromptVisual({
  prompt,
  visual,
}: {
  prompt: string;
  ...
  const cards = visual.numberCards.length ? visual.numberCards : visual.labels;
  const section = getStep57Section(prompt, visual);
  const primary = cards[0] ?? "known values";
  const secondary = cards[1] ?? "model step";
  const tertiary = cards[2] ?? "";
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        ...
        <div style={{ border: "1px solid #bbf7d0", borderRadius: 999, background: "#f0fdf4", color: "#166534", padding: "7px 11px", fontSize: 13, fontWeight: 900 }}>
          Model, calculate, check
        </div>
      </div>

      <div style={{ border: "1px solid #dbeafe", borderRadius: 18, background: "#f8fbff", padding: 14, display: "grid", gap: 10 }}>
        <div style={{ color: "#1e3a8a", fontSize: 13, fontWeight: 900 }}>
          Real-world modelling card
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) auto minmax(0, 1fr)", alignItems: "center", gap: 10 }}>
          <div style={{ border: "2px solid #bfdbfe", borderRadius: 16, background: "#ffffff", color: "#1e3a8a", padding: "14px 10px", textAlign: "center", fontSize: primary.length > 32 ? 16 : 26, fontWeight: 950, lineHeight: 1.15 }}>
            {primary}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 10 }}>
        <div style={{ border: "1px solid #bae6fd", borderRadius: 16, background: "#f0f9ff", padding: 12, color: "#075985" }}>
          <div style={{ fontSize: 12, fontWeight: 950 }}>Remember</div>
          <div style={{ marginTop: 5, fontSize: 14, fontWeight: 800 }}>
            Identify known values, choose operations, and keep units in the answer.
          </div>
        </div>
        <div style={{ border: "1px solid #e0e7ff", borderRadius: 16, background: "#eef2ff", padding: 12, color: "#3730a3" }}>
          <div style={{ fontSize: 12, fontWeight: 950 }}>Unit check</div>
          <div style={{ marginTop: 5, fontSize: 14, fontWeight: 800 }}>
            Compare like with like before choosing a best buy or final decision.
          </div>
        </div>
      </div>
...
export function renderStep57WorksheetOptionCard({
  option,
  selected = false,
}: {
  option: string;
  ...
  const lower = normalized.toLowerCase();
  const label = lower.includes("£") || lower.includes("p")
    ? "Money"
    : lower.includes("km") || lower.includes("ml") || lower.includes("litre") || lower.includes("kg") || lower.includes("m^2")
      ? "Units"
      : lower.includes("pack") || lower.includes("store") || lower.includes("bag")
        ? "Best buy"
        : lower.includes("bus") || lower.includes("day") || lower.includes("hour")
          ? "Time/work"
          : lower.includes("%")
            ? "Percent"
            : "Choice";
...
function getStep58Section(prompt: string, visual: EarlyNumberVisualModel) {
  const text = `${prompt} ${visual.caption} ${visual.numberCards.join(" ")}`.toLowerCase();
  if (text.includes("significant")) return "Significant figures";
  if (text.includes("lower bound") || text.includes("upper bound") || text.includes("bounds") || text.includes("greatest possible")) {
    return "Limits of accuracy";
  }
  if (text.includes("calculate") || text.includes("final answer")) return "Calculate then round";
  if (text.includes("estimate") || text.includes("reasonable")) return "Estimate and check";
  if (text.includes("runner") || text.includes("petrol") || text.includes("rice") || text.includes("mass")) return "Real-world accuracy";
  return "Decimal-place rounding";
}

export function renderStep58WorksheetPromptVisual({
  prompt,
  visual,
}: {
  prompt: string;
  ...
  const cards = visual.numberCards.length ? visual.numberCards : visual.labels;
  const section = getStep58Section(prompt, visual);
  const primary = cards[0] ?? "rounded value";
  const secondary = cards[1] ?? "accuracy";
  const tertiary = cards[2] ?? "";
    ...
    >
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 10 }}>
        <div style={{ border: "1px solid #bae6fd", borderRadius: 16, background: "#f0f9ff", padding: 12, color: "#075985" }}>
          <div style={{ fontSize: 12, fontWeight: 950 }}>Remember</div>
          <div style={{ marginTop: 5, fontSize: 14, fontWeight: 800 }}>
            For bounds, use half the rounding unit on either side of the rounded value.
          </div>
        </div>
        <div style={{ border: "1px solid #e0e7ff", borderRadius: 16, background: "#eef2ff", padding: 12, color: "#3730a3" }}>
...
export function renderStep58WorksheetOptionCard({
  option,
  selected = false,
}: {
  option: string;
