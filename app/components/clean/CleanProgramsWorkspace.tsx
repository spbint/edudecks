...
export default function CleanProgramsWorkspace() {
  ...
  function beginLessonEdit(lesson: CleanProgramLesson) {
            ...
            {view === "create" || (view === "edit" && selectedProgram) ? (
              <>
                ...
                {view === "edit" && selectedProgram ? <section style={cardStyle} aria-labelledby="program-lessons-heading">
                  ...
                  {!editingLessonId ? <form onSubmit={handleLessonSubmit} style={{ display: "grid", gap: 12 }}>
                    <h3 style={{ margin: 0, fontSize: 17 }}>Add lesson</h3>
                    ...
                    <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}><button type="submit" style={buttonStyle} disabled={submitting}>{submitting ? "Saving..." : "Add lesson"}</button></div>
                  </form> : null}
                  ...
                  {lessons.length ? <ol style={{ display: "grid", gap: 10, margin: "24px 0 0", paddingLeft: 24 }}>{lessons.map((lesson, index) => <li key={lesson.id} style={{ paddingLeft: 4 }}><article style={{ border: "1px solid #e2e8f0", borderRadius: 14, padding: 14, display: "grid", gap: 12 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}><div><strong>{lesson.title}</strong>{lesson.estimatedDurationMinutes ? <div style={{ color: "#64748b", marginTop: 4 }}>About {lesson.estimatedDurationMinutes} minutes</div> : null}</div><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><button type="button" style={{ ...buttonStyle, background: "#ffffff", color: "#0f172a" }} onClick={() => void handleMoveLesson(lesson.id, -1)} disabled={submitting || index === 0} aria-label={`Move ${lesson.title} up`}>Move up</button><button type="button" style={{ ...buttonStyle, background: "#ffffff", color: "#0f172a" }} onClick={() => void handleMoveLesson(lesson.id, 1)} disabled={submitting || index === lessons.length - 1} aria-label={`Move ${lesson.title} down`}>Move down</button><button type="button" style={{ ...buttonStyle, background: "#ffffff", color: "#0f172a" }} onClick={() => beginLessonEdit(lesson)} disabled={submitting}>Edit</button><button type="button" style={{ ...buttonStyle, background: "#b91c1c", borderColor: "#b91c1c" }} onClick={() => void handleRemoveLesson(lesson)} disabled={submitting}>Remove</button></div></div>{lesson.instructions ? <p style={{ margin: 0, color: "#475569", lineHeight: 1.6 }}>{lesson.instructions}</p> : null}{editingLessonId === lesson.id ? <form onSubmit={handleLessonSubmit} style={{ display: "grid", gap: 12, paddingTop: 14, borderTop: "1px solid #e2e8f0" }} aria-label={`Edit lesson: ${lesson.title}`}><h3 style={{ margin: 0, fontSize: 17 }}>Edit lesson</h3><label style={{ display: "grid", gap: 6, color: "#334155", fontWeight: 700 }}>Lesson title<input value={lessonTitle} onChange={(event) => setLessonTitle(event.target.value)} style={inputStyle} required /></label><div style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))" }}><label style={{ display: "grid", gap: 6, color: "#334155", fontWeight: 700 }}>Instructions <span style={{ fontWeight: 400 }}>(optional)</span><textarea value={lessonInstructions} onChange={(event) => setLessonInstructions(event.target.value)} style={{ ...textAreaStyle, minHeight: 72 }} /></label><label style={{ display: "grid", gap: 6, color: "#334155", fontWeight: 700 }}>Estimated minutes <span style={{ fontWeight: 400 }}>(optional)</span><input type="number" min="1" value={lessonDuration} onChange={(event) => setLessonDuration(event.target.value)} style={inputStyle} /></label></div><div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}><button type="submit" style={buttonStyle} disabled={submitting}>{submitting ? "Saving..." : "Save changes"}</button><button type="button" style={{ ...buttonStyle, background: "#ffffff", color: "#0f172a" }} onClick={resetLessonForm} disabled={submitting}>Cancel</button></div></form> : null}</article></li>)}</ol> : null}
                </section> : null}
              </>
            ) : null}
