...
export default function FamilySettingsPage() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const { data, error } = await upsertFamilyProfile(userId, settings);

      if (error) {
        setStorageMode("local");
        setSaveError("Settings were saved locally, but the family profile could not be updated in the database.");
        setInitialSettings(settings);
        setSavedAt(new Date().toLocaleString());
        return;
      }
