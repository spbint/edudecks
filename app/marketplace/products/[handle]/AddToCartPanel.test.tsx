...
const product = {
  ...
  description: "Counting bears",
  descriptionHtml: "<p>Counting bears</p>",
  featuredImage: image,
  images: [image],
  variants: [
    {
...
describe("Marketplace add-to-cart inventory controls", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    addLine.mockResolvedValue({ ok: true });
    useMarketplaceCartMock.mockReturnValue({ cart: null, addLine });
  });
  ...
  it("retains the application maximum when inventory is not tracked", () => {
    render(
      React.createElement(AddToCartPanel, {
        product: {
          ...product,
          variants: [{ ...product.variants[0], quantityAvailable: null }],
        },
      }),
    );
  ...
  it("subtracts the quantity already present in the cart", () => {
    useMarketplaceCartMock.mockReturnValue({
      cart: { lines: [{ merchandise: { id: "variant-1" }, quantity: 1 }] },
      addLine,
    });
    render(React.createElement(AddToCartPanel, { product }));
    expect(
      screen.getByText("1 is already in your cart. You can add up to 9 more."),
    ).toBeTruthy();
  ...
  it("resets quantity when the selected variant changes", () => {
    ...
    expect(screen.getByLabelText("Quantity").getAttribute("max")).toBe("2");
  });
});
