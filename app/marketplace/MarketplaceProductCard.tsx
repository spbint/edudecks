...
export default function MarketplaceProductCard({
  ...
  const category =
    product.productType && !/^[a-z0-9.-]+\.[a-z]{2,}(?:\/|$)/i.test(product.productType)
      ? product.productType
      : "Learning resource";
  return (
    <article className="marketplace-product-card">
      <Link
        ...
        href={`/marketplace/products/${encodeURIComponent(product.handle)}`}
      ...
      <div className="marketplace-product-card-body">
        ...
        <h3>
          <Link href={`/marketplace/products/${encodeURIComponent(product.handle)}`}>
            {product.title}
          </Link>
        </h3>
        <div>
          <span className="marketplace-price">
            {Number(price.amount) === 0 ? "Free" : formatShopifyMoney(price)}
            {Number(price.amount) !== 0 && hasRange ? " and up" : ""}
          </span>
        </div>
        {!product.availableForSale ? (
          <div className="marketplace-product-meta" role="status">
