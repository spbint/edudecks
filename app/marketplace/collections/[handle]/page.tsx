...
export default async function CollectionPage({ params }: { params: Promise<{ handle: string }> }) {
  ...
  return <main className="marketplace-main"><div className="marketplace-page-heading"><Link className="marketplace-link" href="/marketplace/collections">All collections</Link><h1>{collection.title}</h1>{collection.description ? <p>{collection.description}</p> : null}</div>{collection.products.length ? <div className="marketplace-product-grid">{collection.products.map((product) => <MarketplaceProductCard key={product.id} product={product} />)}</div> : <div className="marketplace-state">This collection is empty right now.</div>}</main>;
}
